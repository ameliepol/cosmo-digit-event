class MailerScheduler < ParticipantMailer
  ...
  namespace :mailer_scheduler do
    ...
    task :send_emails_to_participants_before_event => :environment do
      alert_one_month_before = (DateTime.now - 2).to_date
      alert_fifteen_days_before = (DateTime.now - 1).to_date
      alert_seven_days_before = (DateTime.now - 1).to_date
      alert_one_day_before = (DateTime.now - 1).to_date
